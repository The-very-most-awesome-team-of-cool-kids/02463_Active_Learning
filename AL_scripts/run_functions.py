...
from query_strategies import RandomSampling, UncertaintySampling, MarginSampling
import torch
import pickle
# import random


def train_rounds(X_te, Y_te, n_rounds, n_query, n_train, ids_labeled, strategy, dataset, seed):
    ...
    # save results
    with open("AL_scripts/results/"+dataset+ "_" +strategy.get_name()+ "_accuracies_test" +".pkl", "wb") as file:
...
def run_AL(n_train, n_query, n_rounds, dataset, strat, seed = 1):
    ...
    train_rounds(X_te, Y_te, n_rounds, n_query, n_train, ids_labeled, strategy, dataset, seed)
